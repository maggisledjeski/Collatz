...
num = max   #is the current sequence number

#uses the max to stop the while loop until every sequence number starting from in is tested
while max != 1 do
    num = max   #sets num to the max (current sequence number), to make sure the current sequence number is being tested
    steps = 0   #step count

    #compute the sequence count until num = 1
    while num != 1 do
    ...
    num = max   ##resets num to be equal to the current sequence number
    ...
    max = max - 1   #decrements the current sequence number
end
