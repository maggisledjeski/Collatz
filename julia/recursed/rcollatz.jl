...
while j > 1
    ...
    ic = UInt128(0) #sets the step count
    ic = stepCount(i)   #recursive call to get the step count for the current sequence number i
    ...
    for k in 1:10
        ...
        if(sArray[k] == ic) #checks to see if there is a number in the array with the same step count as the current sequence number
            dupF = 1    #sets the duplicate flag to true
